...
Vagrant.configure(2) do |config|
  ...
  config.vm.hostname = "front-end-vol-eng-b"
...
def setup_environment
  return <<-EOS
    ...
    # Add shortcut alias 'fe' to change to project dir
    if ! grep -q 'alias fe=' $VAGRANT_HOME/.bashrc; then
      echo >> $VAGRANT_HOME/.bashrc
      echo 'alias fe="cd /vagrant"' >> $VAGRANT_HOME/.bashrc
    fi
  EOS
end
